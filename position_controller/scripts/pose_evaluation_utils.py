...
def compute_ate(gtruth_file, plan_file):
    gtruth_list = read_file_list(gtruth_file)
    plan_list = read_file_list(plan_file)
    matches = associate(gtruth_list, plan_list, 0, 0.01)
    if len(matches) < 2:
        return False

    gtruth_xyyaw = np.array([[float(value) for value in gtruth_list[a][0:3]] for a,b in matches])
    plan_xyyaw = np.array([[float(value) for value in plan_list[b][0:3]] for a,b in matches])
    
    # Make sure that the first matched frames align (no need for rotational alignment as
    # all the predicted/ground-truth snippets have been converted to use the same coordinate
    # system with the first frame of the snippet being the origin).
#    offset = gtruth_xyyaw[0] - plan_xyyaw[0]
#    plan_xyyaw += offset[None,:]

    # Optimize the scaling factor
    scale = np.sum(gtruth_xyyaw * plan_xyyaw)/np.sum(plan_xyyaw ** 2)
    alignment_error = plan_xyyaw * scale - gtruth_xyyaw
